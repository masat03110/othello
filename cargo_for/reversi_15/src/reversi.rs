...
impl OthelloBoard {
    ...
    pub fn make_1_next_boards2(&self, color: usize) -> Vec<(OthelloBoard,u64,usize,isize)>{
        ...
        //     if total != 0{
        //         let mut times = 0;
        //         let next_board = board.add2(position, color);
        //         let score = next_board.eval_mini_max_sub(color, 1-color, &mut times, 7, 1);
        //         nextboards.push((next_board, position ,1-color , score));
        //     }
        //     position <<= 1;
        // }
    ...
    fn last_mini_max(&self, color: usize, start_time: Instant, timeout: Duration, count:usize)-> usize{
    ...
    fn eval_mini_max_sub(&self, color: usize, turn: usize, times:&mut u64, n:u64, alpha:isize)->isize{
        *times += 1;
        if *times > 500000000{
            return 0;
        }
    ...
    pub fn eval_boards(&self, color: usize, count: usize) -> usize{
        ...
        let timeout_for_final = Duration::from_secs(20);
        if count > 43{//montecalro max 51
                      //a-b max 43 43が1手読める程度
                      //44-45が割れ目か　念のため44-46で計測する必要あり
                      //43-41は不確定切りが必要か
            return self.last_mini_max(color,start_time,timeout_for_final,count);
        }else{
