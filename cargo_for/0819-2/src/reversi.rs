 ...
 **/
 ...
 impl OthelloBoard {
    ...
    fn eval_mini_max_sub3(&self, color: usize, turn: usize, times:&mut u64, n:usize, alpha:isize, memo_eval: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>> ,memo_banmen: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>,memo_banmen_pre: &HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>)->isize{
        *times += 1;
     ...
     fn eval_mini_max_sub2(&self, color: usize, turn: usize, times:&mut u64, n:usize, alpha:isize, memo_eval: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>> ,memo_banmen: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>,memo_banmen_pre: &HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>)->isize{
        *times += 1;
        ...
        let mut beta = isize::MIN;
        let n_1 = n - 1;
        let mut alpha_mode = -1;


        if let Some(value) = (*memo_banmen_pre).get(&(self.black,self.white)){
           if value.is_empty() || n == 0{
                memo_banmen.insert((self.black,self.white),vec![]);
               return self.eval_board_for_last(color);
           }
   
           if value[0].0.white != color as u64{
               return_count = isize::MIN;
               for (board,pos) in value {
                        if alpha_mode == -1{
                            memo.push((OthelloBoard{black:board.black,white:board.white},*pos,isize::MAX));
                            alpha_mode = 0;
                        }
                        else if alpha_mode == 0{

                            now_count = board.eval_mini_max_sub3(color, aite, times, n_1,beta, memo_eval, memo_banmen,memo_banmen_pre);
               ...
               for (board,pos) in value {
                if alpha_mode == -1{
                    memo.push((OthelloBoard{black:board.black,white:board.white},*pos,isize::MIN));
                    alpha_mode = 0;
                }
                else  if alpha_mode == 0{
               ...
               memo.sort_by(|a, b| a.2.cmp(&b.2)); //b>a

           }
           let first_elements: Vec<(_,_)> = memo.iter().map(|(first, second, _)| ((OthelloBoard { black: first.black, white: first.white }),*second)).collect();

           memo_banmen.insert((self.black,self.white),first_elements);
           return return_count;
        }
        else{
           boards = self.make_1_next_boards1_5_2(turn);
     ...
     pub fn eval_boards(&self, color: usize, count: usize, n: &mut usize, mytime: usize, pre_mytime:usize, start_time: Instant, timeout: Duration, memo_banmen: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>, memo_banmen_pre: &HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>, memo_yomikiri: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>,memo_eval: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>,memo_eval_pre: &HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>)-> usize{
         if count > 41{//montecalro max 51
