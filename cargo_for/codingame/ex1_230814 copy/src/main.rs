...
fn mawari(board:u64) -> u64{
    let board1 = board & 0x7F7F7F7F7F7F7F00;
    let board2 = board & 0xFEFEFEFEFEFEFE00;
    let board3 = board & 0x007F7F7F7F7F7F7F;
    let board4 = board & 0x00FEFEFEFEFEFEFE;

    return board1 << 1 | board1 >> 8 | board1 >> 7 | board2 >> 1 | board2 >> 8 | board2 >> 9 |board3 << 1 | board3 << 8 | board3 << 9 | board4 >> 1 | board4 >> 8 | board4 >> 9 ;
}
    ...
    fn eval_board_for_last(&self, color:usize) -> isize{
     ...
     pub fn eval_boards(&self, color: usize, count: usize, n: &mut usize, mytime: usize, pre_mytime:usize, start_time: Instant, timeout: Duration, memo_banmen: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>, memo_banmen_pre: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64)>>, memo_yomikiri: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>,memo_eval: &mut HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>,memo_eval_pre: &HashMap<(u64,u64),Vec<(OthelloBoard,u64,isize)>>)-> usize{
         if count > 39{//montecalro max 51
                       ...
                       //43-41は不確定切りが必要か
             return self.last_eval_mini_max(color, start_time, Duration::from_millis(20000), n, memo_eval, memo_eval_pre,memo_banmen,memo_banmen_pre);
         }else{
...
fn main() {
    ...
    let mut memo_yomikiri_pre:HashMap<(u64,u64), Vec<(OthelloBoard,u64,isize)>> = HashMap::new();
