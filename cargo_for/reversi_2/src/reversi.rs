...
impl OthelloBoard {
    ...
    //     let mut mycolor:u64;
    //     let mut oppcolor:u64;
    //     let mut eval_points:usize;
    //     if color == 0{
    //         // color is black
    //         mycolor = self.black;
    //         oppcolor = self.white;            
    ...
    // 盤面の状態を表示する関数
    pub fn _print(&self, color: usize) {
    ...
    pub fn make_1_next_boards(&self, color: usize, _count: usize) -> Vec<(OthelloBoard,usize,usize)>{
        ...
        for item in valid_pos{
            ...
            next_boards.push((board3,item,turn));
        }   
        return next_boards;
    }
    ...
    fn mini_max_sub(&self, color: usize, turn: usize, count:usize)->isize{
        ...
        for (board, pos, now_turn) in boards {
            if pos == 99{
                return board.subtract(color);
            }
            else if now_turn == color{
                now_count = board.mini_max_sub(color, now_turn,count);
                if return_count == -99{
    ...
    fn mini_max(&self, color: usize, count: usize)-> usize{
        let boards = self.make_1_next_boards(color,count);
        let mut return_pos:usize = 99;
        let mut max:isize = -999;
        let mut now_count;
        for (board, pos, turn) in boards {
            now_count = board.mini_max_sub(color, turn ,count);
            if max < now_count{
                max = now_count;
                return_pos = pos
            }
    ...
    pub fn eval_boards(&self, color: usize, count: usize) -> usize{
        ...
        if count > 52{
            return self.mini_max(color, count);
        }
            ...
            if valids[64] != 0{
                ...
                if count < FIRST{
                    ...
                    if valids_now[item] <= valids[max]{
                        ...
                        eval_num += valids_now[item] - valids[max];

                    }
                }
            }else{
                
                eval_num = 100000*valids_now[item];
            }
