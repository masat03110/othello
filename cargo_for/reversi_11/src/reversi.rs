...
impl OthelloBoard {
    ...
    pub fn make_1_next_boards3(&self, color: usize, flag: u8,count:usize) -> Vec<OthelloBoard>{
        ...
        let mut position:u64 = 1;
        let mut nextboards_sub:Vec<(OthelloBoard,usize)> = vec![];
        let mut flag2:u8 = 0;
        loop{
            ...
            if total != 0{
                flag2 = 1;
                nextboards_sub.push((board.add2(position, color),total));
            }
            position <<= 1;
        }
        ...
        if flag2 != 0{
            if count > 40 {
                nextboards_sub.push((OthelloBoard{ black: 0, white: (1-color) as u64, },99));
                nextboards_sub.sort_by(|a, b| b.1.cmp(&a.1));
            }else{
                nextboards_sub.push((OthelloBoard{ black: 0, white: (1-color) as u64, },0));
                nextboards_sub.sort_by(|a, b| a.1.cmp(&b.1));
            }
            let first_elements: Vec<OthelloBoard> = nextboards_sub.iter().map(|(first, _)| OthelloBoard { black: first.black, white: first.white, }).collect();
            ...
            return board.make_1_next_boards3(1-color, 1,count);
        }
    }
    ...
    pub fn make_1_next_boards2(&self, color: usize, flag: u8) -> Vec<(OthelloBoard,u64,usize,usize)>{
        ...
        loop{
            ...
            if total != 0{
                flag2 = 1;
                nextboards.push((board.add2(position, color),position,1-color,total));
            }
            position <<= 1;
        }
    ...
    fn last_mini_max_sub(&self, color: usize, turn: usize, count:usize, times:&mut u64, novalue:&mut u64)->isize{
        *times += 1;
        if *times > 300000000{
            return 0;
        }
        let mut boards = self.make_1_next_boards3(turn,0,count);
        let mut return_count:isize = -99;
        let mut now_count:isize = -99;

        let check = boards[0].white;
        ...
        if check == 3{
        ...
        else if 1-check == color as u64{
            for board in boards {
                    now_count = board.last_mini_max_sub(color, 1-color,count+1, times, novalue);
            ...
            for board in boards {
                    now_count = board.last_mini_max_sub(color, color,count+1, times, novalue);
    ...
    fn last_mini_max(&self, color: usize, count: usize)-> usize{
        ...
        for (board, pos, turn,_) in boards {
            novalue = 0;
            now_count = board.last_mini_max_sub(color, turn ,count+1, &mut times, &mut novalue);
