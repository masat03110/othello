...
impl OthelloBoard {
    ...
    pub fn make_1_next_boards2_5(&self, color: usize, n:usize) -> Vec<(OthelloBoard,u64,usize,isize,u64)>{
        ...
        for _ in 0..k {
            ...
            let score = next_board.eval_mini_max_sub2(color, 1-color, &mut times, n,1);
            println!("score:{}",score);
    ...
    fn eval_board_for_last(&self, color:usize) -> isize{
        ...
        if color == 0{
            ...
            my_kakutei_board = (self.add2(akikoma, oppcolor)).add2(akikoma, oppcolor).black;
            opp_kakutei_board = (self.add2(akikoma, color)).add2(akikoma, color).white;
            ...
            my_kakutei_board = (self.add2(akikoma, oppcolor)).add2(akikoma, oppcolor).white;
            opp_kakutei_board = (self.add2(akikoma, color)).add2(akikoma, color).black;
            my_kakutei = count_pieces(my_kakutei_board);
            opp_kakutei = count_pieces(opp_kakutei_board);
            if myvalid | oppvalid == 0{
                if self.subtract(color) > 0{
        ...
        eval_point = eval_point + myv*10 - oppv*10;

        // if oppv < 4{
        //     eval_point += 300 - 50*oppv;
        // }
        ...
        // let count = count_pieces(akikoma);

        // if my_kakutei + opp_kakutei != 0{
        ...
        if myv > 3 && eval_point > -30{
            ...
            eval_point -= kado_oppcount*250;
        }
        
        // }else if count > 20{
        //     eval_point = eval_point + kakuteisa*20 - oppv*20;
        // }else{
        //     eval_point = eval_point + kakuteisa*30;
        // }

        eval_point = eval_point + (1000*my_kakutei/count_pieces(myboard));

        eval_point = eval_point - (1000*opp_kakutei/count_pieces(oppboard));

        if opp_kakutei_board & 0xFF818181818181FF != 0{
            eval_point -= 300;
        }
