...
impl OthelloBoard {
    ...
    pub fn make_1_next_boards2(&self, color: usize, flag: u8) -> Vec<(OthelloBoard,u64,usize,isize)>{
        let board = OthelloBoard {
            black: self.black,
            white: self.white,
        };
        ...
        loop{
            ...
            if total != 0{
                flag2 = 1;
                let mut times = 0;
                nextboards.push((board.add2(position, color),position,1-color,self.eval_mini_max_sub(color, 1-color, &mut times, 5)));
            }
            position <<= 1;
        }
    ...
    fn eval_mini_max_sub(&self, color: usize, turn: usize, times:&mut u64, n:u64)->isize{
        *times += 1;
        if *times > 500000000{
            return 0;
        }
        ...
        if let Some(OthelloBoard{ black: _, white: check}) = boards.pop() {
            ...
            if 1-check == color as u64{
                return_count = -2000;
                for board in boards {
                        now_count = board.eval_mini_max_sub(color, 1-color, times, n-1);
                        
                        if return_count < now_count {
                            return_count = now_count;
                        }else if now_count > 2000{
                            return now_count;
                        }
                }
                ...
                return_count = 2000;
                for board in boards {
                        now_count = board.eval_mini_max_sub(color, color, times, n-1);
    ...
    fn eval_mini_max(&self, color: usize)-> usize{
        let boards = self.make_1_next_boards2(color,0);
        let mut position:u64 = 0;
        let mut return_pos:usize = 0;
        let mut max:isize = -99999999;
        let mut now_count;
        let mut times:u64 = 0;
        for (board, pos, turn,_) in boards {
            now_count = board.eval_mini_max_sub(color, turn , &mut times, 5);
    ...
    pub fn eval_boards(&self, color: usize, count: usize) -> usize{
        if count > 42{//montecalro max 51
            ...
            return self.eval_mini_max(color);
        }
